...
def fit_classifier():
    ...
    print("in fit classifier line 49")
...
############################################### main
root_dir = 'C:/Users/pande/Documents/Programming/raganet/InceptionTime/root_dir'
xps = ['use_bottleneck', 'use_residual', 'nb_filters', 'depth',
       'kernel_size', 'batch_size']

if sys.argv[1] == 'InceptionTime':
    print("Running InceptionTime")
    ...
    for iter in range(nb_iter_):
        ...
        for dataset_name in utils.constants.dataset_names_for_archive[archive_name]:
            ...
            print('******************')
            print('******************')
            # x_train = normalized(x_train, 0)
            # scaler = sklearn.preprocessing.Normalizer().fit(x_train)
            # x_train = scaler.transform(x_train)

            # scaler2 = sklearn.preprocessing.Normalizer().fit(x_test)
            # x_test = scaler.transform(x_test)
            print(x_train)
            # tf.compat.v1.disable_eager_execution()
            # x_train = tft.scale_to_z_score(tf.cast(x_train, tf.float32))
            # x_test = tft.scale_to_z_score(tf.cast(x_test, tf.float32))
